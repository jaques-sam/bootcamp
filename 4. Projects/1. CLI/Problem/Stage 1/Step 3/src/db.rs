...
impl JiraDatabase {
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        db.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("could not find epic id={epic_id}"))?.stories.push(db.last_item_id);
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        for story_id in &db.epics.get(&epic_id).ok_or_else(|| anyhow!("could not find epic id={epic_id}"))?.stories {
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        let epic = db.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("could not find epic id={epic_id}"))?;
        let story_idx = epic.stories.iter().position(|id| id == &story_id).ok_or_else(|| anyhow!("story id={story_id} not found in epic stories vector"))?;
    ...
    pub fn update_epic_status(&self, epic_id: u32, status: Status) -> Result<()> {
        let mut db = self.read_db()?;
        db.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("could not find epic id={epic_id}"))?.status = status;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        let mut db = self.read_db()?;
        db.stories.get_mut(&story_id).ok_or_else(|| anyhow!("could not find epic id={story_id}"))?.status = status;
